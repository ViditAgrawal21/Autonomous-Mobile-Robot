...
bool comm_check_ = false;
bool prev_comm_check_ = false;
...
void check_callback(const void * msgin)
{  
  if(msgin == NULL){
    return;
  }
  ...
  comm_check_ = msg->data;
}

void communication_check(){
  if((comm_check_ == prev_comm_check_)){   // && (millis() - prev_time > 2000)   && (count == 0)
    // digitalWrite(25, LOW);
    comm_err_flag = true;
    // prev_time = millis();
    v1 = 0;
  ...
  else if(comm_check_ != prev_comm_check_){
    // digitalWrite(25, HIGH);
    comm_err_flag = false;
    // prev_time = millis();
    count = 0;
  }
  prev_comm_check_ = comm_check_;
...
void ping_agent() {
  // Ping the agent to check if it is reachable
...
void setup() {
  ...
  RCCHECK(rclc_executor_add_subscription(&executor, &check_sub_, &check_, &check_callback, ON_NEW_DATA));
...
void loop() {
  ...
  //   // digitalWrite(25, LOW);
  // }
  // if(prev_comm_check_ == true){
  //   digitalWrite(25, HIGH);
  // }
  // else if(prev_comm_check_ == false){
  //   digitalWrite(25, LOW);
  // }
  moveMotors();
  ...
  rcl_ret_t rc = rclc_executor_spin_some(&executor, RCL_MS_TO_NS(1));
  if(rc != RCL_RET_OK){
    digitalWrite(25, HIGH);
  }
}